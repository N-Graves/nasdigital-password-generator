...
const JS = new URL("../dist/password-generator.js", import.meta.url);
const CSS = new URL("../dist/password-generator.css", import.meta.url);
...
const foreign = [...new Set(selectors.map((s) => s.trim()))].filter((s) => !s.startsWith(".pwg"));
check(foreign.length === 0, "defines only .pwg- classes", foreign.join(" "));
